...
void AITuner::setAutoCalibrate(bool enabled) {
    ...
    if (!enabled) {
        ...
        state.calibrationAccumulatedReward = 0.0;
    }
}
...
void AITuner::provideFeedback(const AimbotTarget& target, double mouseX, double mouseY) {
    ...
    double reward = calculateReward(target, mouseX, mouseY, config.targetRadius);
    ...
    if (state.calibrating) {
        ...
        state.calibrationAccumulatedReward += reward;

        const int samplesPerSetting = std::max(1, state.calibrationSamplesPerSetting);
        if (state.calibrationSamples >= samplesPerSetting) {
            double averageReward = state.calibrationAccumulatedReward /
                                   static_cast<double>(state.calibrationSamples);
            if (averageReward > state.bestReward) {
